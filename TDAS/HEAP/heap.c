...
//Pre:
//Post: Devuelve true solo en caso de que se trate de un heap con funcion de heap. 
bool es_heap_valido(heap_t* heap){
	if(!heap)
		return false;
	if(!(heap->funcion_heap))
		return false;
	return true;
}
 ...
 */
abb_t* heap_crear(abb_comparador comparador, abb_liberar_elemento destructor){
	if(!comparador)
		return NULL;
